...
def test_search_rerank_uses_cache_when_available(monkeypatch):
    async def fake_get_json(self, key: str):
        ...
        return {
            ...
            "strategy": "hybrid",
            "retrieval_limit": 5,
            "reranked_hits": [{"id": "cached-1", "text": "cached"}],
        }

    async def fail_search(self, query: str, limit: int = 5):
        _ = (self, query, limit)
        raise AssertionError("EmbeddingsService.search should not be called on cache hit")

    monkeypatch.setattr("app.services.cache.CacheService.get_json", fake_get_json)
    monkeypatch.setattr("app.services.embeddings.EmbeddingsService.search", fail_search)
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["reranked_hits"][0]["id"] == "cached-1"
